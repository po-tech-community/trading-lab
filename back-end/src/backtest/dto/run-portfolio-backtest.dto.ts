...
export class PortfolioAssetDto {
  @ApiProperty({
    example: 'BTC',
    description: 'Asset symbol (e.g. BTC, ETH, AAPL, TSLA)',
  })
  @IsString()
  @IsUppercase({
    message: 'Symbol must be uppercase (e.g. BTC, ETH, AAPL, TSLA)',
  })
  symbol: string;
  ...
  @ApiProperty({
    example: 60,
    description:
      'Weight percentage for this asset (e.g. 60 = 60%). All weights must sum to 100.',
  })
  @Type(() => Number)
  @IsNumber()
...
export class RunPortfolioDcaBacktestDto {
  @ApiProperty({
    ...
    description:
      'List of assets with their percentage weights. Weights must sum to 100.',
    example: [
  ...
  @ApiProperty({
    example: 100,
    description: 'Total amount to invest per period in USD (must be > 0)',
  })
  @Type(() => Number)
  @IsNumber()
  ...
  @ApiProperty({
    ...
    description:
      'End date as epoch milliseconds (UTC midnight). Must be after startDate.',
  })
  @Type(() => Number)
  @IsNumber()
  @IsInt()
  ...
  @ApiProperty({
    required: false,
    description:
      'Optional take profit and stop loss triggers (applied to whole portfolio)',
    example: {

...
| **L2 Portfolio DCA** | Done (3/3)                     | Done (3/3) | Complete |
| **L3 Triggers**      | Done (6/6)                     | Done (6/6) | Complete |
| **L4 AI Adapt**      | Done (5/7) — L4-BE-4/7 pending | Done (5/5) | ~75%     |
...
| L2-FE-2 | **Composition pie chart** – Pie chart of initial allocation (weights). Use same chart library as project (e.g. Recharts).                                                                                                                                             | S     | L2-FE-1          | Hoàng      | Done | `CompositionPieChart.tsx` integrated in `PortfolioPage.tsx`; reads weights from `result.summary.assets`.                                                                      |
| L2-FE-3 | **Portfolio results view** – Reuse/adapt summary + timeline chart for portfolio. Add per-asset breakdown: table or small bars for each symbol's ROI/contribution.                                                                                                     | M     | L2-BE-3, L2-FE-1 | Hoàng      | Done | `AssetBreakdown.tsx`, `PortfolioTrajectoryChart`, `SummaryStatsCards`, `TradeHistoryTable` all integrated and wired to API response in `PortfolioPage.tsx`.                   |
...
| L4-BE-4 | **Chat session / history** – Store and retrieve conversation per backtest/session so user can ask follow-ups with same context.                                                                                                                             | M     | L4-BE-3          | Kiệt       | -    |
| L4-BE-7 | **Backtest context persistence for AI** – Persist a compact backtest snapshot (config + summary + key trades, optional sampled timeline) or a backtest result reference id so AI chat can bind to a stable run instead of fire-and-forget payload only.     | M     | L3-BE-6, L4-BE-3 | Kiệt       | -    |
| L4-BE-5 | **MCP setup & registry** – Set up MCP client runtime, provider registry, tool discovery, environment configuration, and permission policy (allow/deny list + audit metadata). Include timeout/retry defaults and fallback strategy.                         | M     | L4-BE-3          | Kha        | Done | `McpRuntimeService` (644 lines), `McpConfigService`, `McpPermissionService`, `McpProviderRegistry`, `mcp.types.ts` — full env-driven config with timeout/retry/fallback. PR #47. `POST /ai/mcp/inspect` + `POST /ai/mcp/execute` added for 2-step approval flow (L4-FE-4). |
| L4-BE-6 | **MCP domain tools integration** – Add concrete MCP-powered tools for AI Advisor (market snapshot, portfolio diagnostics, risk checks), map tool outputs into model context, and return tool-backed evidence in final advice payload.                       | L     | L4-BE-5          | Kha        | Done | `backtest-context.server.ts`, `market-snapshot.server.ts`, `portfolio-diagnostics.server.ts`; evidence[] returned in `/ai/analyze` response with `structuredContent`. PR #47.                                                                                              |
| L4-FE-1 | **AI Advisor panel** – Slide-over or modal: “Consult AI Advisor” opens panel. Show current backtest summary in panel header or first message.                                                                                                               | S     | L1-FE-2, L1-FE-3 | Quân H     | Done | `AiAdvisorPanel.tsx` — slide-over Sheet opened from `DcaBacktestPage`; backtest summary shown in header. PR #46.                                                                                                                                                           |
| L4-FE-2 | **Chat UI** – Input for user message; display assistant reply with markdown rendering. Loading state while request in flight. Optional: stream response for better UX.                                                                                      | M     | L4-BE-3          | Quân Huỳnh | Done | Chat UI với markdown renderer tự viết, loading spinner, wired vào `/ai/analyze`. PR #46.                                                                                                                                                                                   |
| L4-FE-3 | **Suggested actions** – If API returns `suggestedActions`, show as chips or buttons (e.g. “Increase frequency to Weekly”) that can prefill the form or scroll to config.                                                                                    | S     | L4-FE-2          | Quân H     | Done | Chips hiển thị đúng từ `suggestedActions[]`. Deviation: click chỉ show toast notification, chưa prefill form field trực tiếp. PR #46.                                                                                                                                      |
| L4-FE-4 | **MCP setup UX (approval + trace)** – Add MCP execution panel in chat to show tool name/purpose/input preview, approve/deny sensitive actions, and display tool execution status/errors.                                                                    | M     | L4-BE-5, L4-FE-2 | Hoàng      | Done | `McpExecutionPanel.tsx` với approve/deny/status UI. Deviation: chạy với mock/simulate flow (setTimeout), chưa wire vào BE response vì BE không có pre-approval step. PR #49.                                                                                               |
| L4-FE-5 | **MCP feature UX (result cards)** – Render structured MCP outputs as reusable cards/widgets (market snapshot, risk check, allocation diagnostics) and connect them with suggested actions in chat flow.                                                     | M     | L4-BE-6, L4-FE-4 | Hoàng      | Done | `ResultCards.tsx` với 3 loại card. Deviation: data từ `mcp-mock-data.ts` (random), chưa đọc từ `evidence[]` trả về bởi `/ai/analyze`. PR #49.                                                                                                                              |
...
## Chat Enhancements (Post-Sprint 4)

These improvements are identified after initial AI chat delivery. Assign to available developers in a follow-up sprint.

| ID    | Task                                                                                                                                                                                                                                                             | Scope | Deps             | Assignee                | Done |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ---------------- | ----------------------- | ---- |
| ENH-1 | **AI streaming responses** – Replace single-shot `POST /ai/analyze` with a Server-Sent Events endpoint (`GET /ai/analyze/stream`). FE reads the stream incrementally and appends tokens to the assistant message as they arrive, eliminating the wait spinner.   | L     | L4-BE-3, L4-FE-2 | Kiệt (BE) + Quân H (FE) | -    |
| ENH-2 | **FloatingAiChat backtest context** – `FloatingAiChat` currently sends queries without any backtest context. Pass the most-recently-completed backtest result (summary + trades) into the chat so the AI can give context-aware advice from the floating widget. | S     | L4-FE-1, L4-FE-2 | Quân H                  | -    |
| ENH-3 | **Panel-to-fullscreen context preservation** – When the user collapses/expands `AiAdvisorPanel` or navigates between DCA and Portfolio pages the message history resets. Lift chat state to a context/store so the conversation persists across layout changes.  | M     | L4-FE-1, L4-FE-2 | Quân H                  | -    |

---

## AI Advisor Studio (`/home/ai-advisor`) — Not Implemented

The route exists and `AiChatPage` is wired, but the following items are placeholder/stub and need real implementation.

### Entry point

| ID      | Task                                                                                                                                                                                                                                                   | Scope | File           | Assignee | Done |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- | -------------- | -------- | ---- |
| AI-FE-1 | **Unlock AI Advisor on HomePage** – `quickActions` has `enabled: false` for AI Advisor, showing "Coming soon" badge and a disabled button. Flip to `enabled: true`, update `usage` text, and remove the disabled state from the bottom intro card too. | XS    | `HomePage.tsx` | Quân H   | -    |

### `AiChatPage.tsx` — logic gaps

| ID      | Task                                                                                                                                                                                                                                                                                                                                                       | Scope | File                            | Assignee | Done |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------------------------------- | -------- | ---- |
| AI-FE-2 | **Inject backtest context into AI Studio chat** – `handleSend` calls `mcpInspect({ userQuery })` and `analyzeBacktest({ userQuery })` with no `backtestContext`. Add a "Load backtest" selector (dropdown or session picker) so the user can attach their latest DCA or portfolio result; pass the selected result as `backtestContext` to every API call. | M     | `AiChatPage.tsx`, `Sidebar.tsx` | Hoàng    | -    |
| AI-FE-3 | **Conversation history in AI Studio** – Like `AiAdvisorPanel` (Fix #1), `AiChatPage.handleSend` sends only the raw query with no prior turns. Before each API call, collect the last 6 non-system messages, serialize them as `"User: …" / "Assistant: …"`, and prepend to `userQuery`.                                                                    | S     | `AiChatPage.tsx`                | Hoàng    | -    |
| AI-FE-4 | **Error handling in AI Studio** – Both `handleSend` catch and `runAfterDecisions` catch show a generic string. Distinguish HTTP 401 ("please log in"), 429 ("too many requests"), 503 ("AI not configured"), and generic fallback — same pattern applied to `AiAdvisorPanel` (Fix #3).                                                                     | S     | `AiChatPage.tsx`                | Hoàng    | -    |

### `ChatPanel.tsx` — UI gaps

| ID      | Task                                                                                                                                                                                                                                                                                                                                                                                         | Scope | File                                  | Assignee | Done |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------------------------------------- | -------- | ---- |
| AI-FE-5 | **Markdown rendering in ChatPanel messages** – `msg.text` is rendered as a plain string inside a `<div>`. `AiAdvisorPanel` already has a `MarkdownContent` component (bold, italic, headings, lists, inline code). Extract `MarkdownContent` to a shared component and use it for AI messages in `ChatPanel`.                                                                                | S     | `ChatPanel.tsx`, `AiAdvisorPanel.tsx` | Quân H   | -    |
| AI-FE-6 | **Wire ResultCard action buttons** – All six action callbacks on the result cards are `console.log` stubs: `onViewDetails`, `onAddToWatchlist` (MarketSnapshotCard); `onViewReport`, `onAdjustStrategy` (RiskCheckCard); `onRebalance`, `onViewAllocation` (AllocationDiagnosticsCard). Define real actions: e.g. navigate to the relevant backtest page, open a modal, or scroll to config. | M     | `ChatPanel.tsx`, `ResultCards.tsx`    | Kha      | -    |

### `Sidebar.tsx` — static mock

| ID      | Task                                                                                                                                                                                                                                                                                                                                                      | Scope | File                             | Assignee               | Done |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | -------------------------------- | ---------------------- | ---- |
| AI-FE-7 | **"New investigation" clears chat** – The "New investigation" button renders but does nothing. Wire it so it resets the `useMcpChat` message list (add a `clearMessages` action to the hook) and focuses the input. No backend session needed — just in-memory reset.                                                                                     | S     | `Sidebar.tsx`, `use-mcp-chat.ts` | Hoàng                  | -    |
| AI-FE-8 | **Real session history sidebar** – Currently shows 3 hardcoded mock items and a non-functional search input. Options: (a) store sessions in `localStorage` (title = first user message, timestamp), load on mount, allow switching; or (b) add a BE endpoint `GET /ai/sessions` và `POST /ai/sessions`. The search input should filter the rendered list. | L     | `Sidebar.tsx`                    | Kiệt (BE) + Hoàng (FE) | -    |

### `router.tsx` — stub sub-routes

| ID       | Task                                                                                                                                                                                                                                                          | Scope | File         | Assignee                                            | Done |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------------ | --------------------------------------------------- | ---- |
| AI-FE-9  | **`/home/ai-advisor/create-model` page** – Currently renders `<div className="p-8">Create Model Page</div>`. Define what "create model" means (custom prompt template? saved strategy config?) and implement the page.                                        | M     | `router.tsx` | Quân Trương                                         | -    |
| AI-FE-10 | **`/home/ai-advisor/settings` page** – Currently renders `<div className="p-8">AI Settings Page</div>`. Implement: API key status, model selection (if configurable), rate limit info, enable/disable MCP tools. > ***Note: If MCP tool toggling is needed, coordinate with Kha on the data structure before implementing.*** | M     | `router.tsx` | Quân Trương | -    |

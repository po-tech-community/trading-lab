...
export function AssetList({ fieldArrayName = "assets" }: AssetListProps) {
  ...
  const handleAdd = useCallback(() => {
    ...
    const nextSymbol =
      SUPPORTED_SYMBOLS.find((s) => !usedSymbols.includes(s.value))?.value ??
      "BTC";
  ...
  return (
    <div className="space-y-3">
      {/* ── Column headers ── */}
      <div className="grid grid-cols-[minmax(0,1fr)_100px_36px] gap-2 px-1">
      ...
      <div className="space-y-2">
        {fields.map((field, index) => {
          ...
          return (
            <div
              key={field.id}
              className="grid grid-cols-[minmax(0,1fr)_100px_36px] gap-2 items-start"
            >
              {/* Symbol selector */}
              <FormField
                control={form.control}
                ...
                render={({ field: f }) => (
                  <FormItem className="space-y-1 min-w-0 w-full">
                    <Select onValueChange={f.onChange} value={f.value}>
                      <FormControl>
                        <SelectTrigger className="h-9 w-full bg-background/50 border-border/60 hover:border-primary/40 transition-colors font-medium text-sm">
      ...
      */}
      ...
      <input
        type="hidden"
        data-testid="weight-valid"
        data-valid={isWeightValid}
      />
    </div>
  );
}
